...
v0.5 executes the core Exchange/Runtime Pack operations plus `verify_runtime_profiles` against `kristal.v5:runtime-pack-portable-conformance@1`. `verify_signature` and `verify_trust` are exercised with ephemeral Ed25519/trust fixtures generated by the harness. RP-2 through RP-5 are now executable implementation-conformance checks rather than WARN-only coverage.
